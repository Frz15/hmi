...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh) : data_(new rslidar_rawdata::RawData())
{
  ...
  private_nh.param("output_points_topic", output_points_topic, std::string("rslidar_points_hmi"));
  output_ = node.advertise<sensor_msgs::PointCloud2>(output_points_topic, 10);

  srv_ = boost::make_shared<dynamic_reconfigure::Server<rslidar_point_convert_hmi::ConvertHMINodeConfig> >(private_nh);
  dynamic_reconfigure::Server<rslidar_point_convert_hmi::ConvertHMINodeConfig>::CallbackType f;
...
void Convert::callback(rslidar_point_convert_hmi::ConvertHMINodeConfig& config, uint32_t level)
{
